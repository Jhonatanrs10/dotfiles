...
dir_for_server_file="$HOME/Servers/PalServer"
...
[1]Update/Install"
	read -r choice2
	case "$choice2" in
	1) steamcmd +@sSteamCmdForcePlatformType linux +force_install_dir "$dir_for_server_file" +login anonymous +app_update 2394010 validate +quit ;;
	*) echo "Saindo..." ;;
	esac
}

backup_server() {
	mkdir -p PalworldServerBKP/Pal/
	cp -r Pal/Saved PalworldServerBKP/Pal/
}
