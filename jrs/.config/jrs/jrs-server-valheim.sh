...
dir_for_server_file="$HOME/Servers/ValServer"
...
run_server() {
	...
	./valheim_server.x86_64 -nographics -batchmode -name "Asgard (JR)" -public 0 -savedir "$dir_for_server_file/World" -port 2456 -world "Asgard" -password "1234e5" -crossplay -preset default
...
[1]Update/Install"
	read -r choice2
	case "$choice2" in
	1) steamcmd +@sSteamCmdForcePlatformType linux +force_install_dir "$dir_for_server_file" +login anonymous +app_update 896660 validate +quit ;;
	*) echo "Saindo..." ;;
	esac
